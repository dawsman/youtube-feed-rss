...
def tag_channel(db_path: Path, name_query: str, add_tags: str = None, remove_tags: str = None):
    """Add or remove tags from a channel."""
    conn = sqlite3.connect(db_path)
    
    cursor = conn.execute("SELECT id, name, tags FROM channels WHERE name LIKE ?", (f"%{name_query}%",))
    rows = cursor.fetchall()
...
def main():
    ...
    parser.add_argument("--db", help="Database path (default: ~/.config/youtube-feed/feed.db)")
    parser.add_argument("--output", help="Output directory (default: ~/.config/youtube-feed/)")
